...
7. **Desenvolvimento da interface (Streamlit)**  
   Construí a aplicação web com filtros e visualização do ranking.  
   A IA auxiliou na estruturação do código e resolução de erros durante o desenvolvimento.

8. **Deploy e ajustes em produção**  
   Realizei o deploy via Streamlit Cloud e resolvi problemas de ambiente (ex: dependências).  
   A IA foi utilizada para troubleshooting e identificação rápida de erros.
